...
class Q-learning(nn.Module):
    def __init__(self, sigma=0.4, hidden=16):
        super(Q-learning, self).__init__()
...
policy_net = Q-learning(sigma=sigma).to(device)
target_net = Q-learning(sigma=sigma).to(device)
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if done:
            ...
            #plot_durations()
            break
    # Update the target network, copying all weights and biases in Q-learning
